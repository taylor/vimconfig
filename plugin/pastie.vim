...
function! s:cookies()
    ...
    if !exists("g:pastie_account")
        ...
        let rubycmd = rubycmd . '.each {|dir| Dir[File.join(File.expand_path(dir),%{*})].select {|p| File.exists?(File.join(p,%{cookies.txt}))}.each {|p| File.open(File.join(p,%{cookies.txt})).each_line { |l| a=l.split(9.chr); puts [a[4],a[6]].join(%{ }) if a[0] =~ /pastie\.org#{36.chr}/ && Time.now.to_i < a[4].to_i && a[5] == %{account} }}}'
        let output = ''
